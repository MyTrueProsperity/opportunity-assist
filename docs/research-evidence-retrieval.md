...
- **Traceable.** Each research fact carries `selection` (rank, package, record id, reasons). The strategy instructions require `evidence_chain` to cite only supplied records and to state what each supports and does not support. A strategy that names a library record it was not given is rejected and nothing is saved. `app.content.strategy_evidence` records what was selected, why, the request size, and which selected records the strategy cited (`cited_records`).
- **Citation allowlist.** The only research a strategy may cite is the research facts selected as evidence for that request (`SE.researchCitations`). A record id that reaches the model only through claim rules, methodology, the planning crosswalk, narrative guidance, quarantine notes, funder packets, statistics, aliases, selection reasons or other prompt text is not supplied evidence: citing it fails the job with `EVIDENCE_CHAIN`. Every string section is checked. Where the strategy names a package version beside a record id, the exact package/record pair must be selected; a legacy alias is never a selected record. Organization facts and methodology ids are not research citations and are unaffected. (Found in production on 2026-09-26: claim rule EM-CR-02 named EM-051, which was not selected; the model cited it with a figure that was not in the request, and the previous check, which accepted any id appearing anywhere in the request, passed it.)
- **Quantitative grounding.** Strategy may state an applicant-specific amount, rate, quantity, staffing level, duration, unit cost, budget allocation or calculation only when it is supported by what was supplied: organization facts, the program, the application (funder and opportunity details) and its questions, or a cited selected research fact whose finding it is (`strategy-quantities.js`). Claim rules, methodology, the planning framework and selection reasons are not support. An organization or application figure must also be about the same subject as the claim (a "$5,000 scholarship" does not support a "$5k facility budget"). A calculation is allowed only when every input is supported, and its arithmetic is checked. Bare numbers (section and list numbers, dates, years, grades, record ids) are not checked. An unsupported figure fails the job with `UNSUPPORTED_QUANTITY` and nothing is saved; the prompt tells the model to name a missing input as a gap instead of estimating it. (Found in production on 2026-09-26: $15/hour, 10 hours/week, a 36-week year, ~$810,000, a $300,000 wage budget, $25k to $55k salaries, 0.5 to 1.0 FTE and $1,500+/course, none of them supplied.)
- **Concise sections.** Each strategy section has a word guide in the tool schema (about 2,850 words in all), and evidence_gaps and budget_consistency are short lines under Established, Gaps, and Decisions and actions. The 12,000-token output allowance is unchanged.
...
### Strategy runs as a background job

Strategy writes nine substantial sections, which takes longer than a synchronous web request allows (production timed out at about 30 seconds while the model was still writing). Choosing Generate strategy now queues a job and returns immediately:

- **Queue.** The `strategy` action checks the application revision and program, then calls `gf_strategy_job_enqueue` (`supabase/grant-factory/20260926004842_strategy_jobs.sql`). One job can be QUEUED or RUNNING per application (a partial unique index), so repeated clicks join the active job. States follow `source_jobs`: QUEUED, RUNNING, COMPLETED, FAILED.
- **Run.** The request dispatches `netlify/functions/grant-factory-strategy-background.js` on the same deployment with the caller's session. The worker re-authenticates that session, re-checks Grant Factory membership for the job's workspace, claims the job with a 7-minute lease and runs the same bounded evidence selection, size guard and model call as before (strategy may write up to 12,000 output tokens, with a 300-second model timeout; other tasks keep 4,500 tokens and 45 seconds). A response that reaches its output allowance is incomplete and is rejected, never saved.
- **Save atomically.** Nothing is saved unless the complete response passes schema validation and the citation allowlist, the strategy inputs (funder details, program, questions and approved facts) are unchanged since the job was queued (an input hash), and the lease is still held. The save uses `gf_save_application`, which also rejects concurrent edits. A failed, stale or expired job leaves the existing strategy untouched.
- **Status.** The browser polls `strategy_status`, which reads only the job row (no research or application load), and loads the application once when the job completes. Leaving or refreshing the page does not stop the job; opening the application reconnects. A queued job that has not started is dispatched again after 45 seconds.
- **Timeouts and recovery.** A RUNNING job past its lease is marked FAILED (TIMEOUT) and a QUEUED job never started within 10 minutes is marked FAILED (NOT_STARTED); both can be retried. AI run records left RUNNING for over 15 minutes (the longest any function can run) are marked FAILED as abandoned; completed runs and their token counts are never changed.
- **Usage.** The model call is logged in `gf_ai_runs` as before, including the provider's token usage for responses that were returned but rejected (output limit reached or invalid structure), which stay FAILED. The job row records model, input and output tokens, request size and the selected record ids and reasons, never research text.
