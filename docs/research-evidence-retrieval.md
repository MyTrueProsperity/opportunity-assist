...
- **Run.** The request dispatches `netlify/functions/grant-factory-strategy-background.js` on the same deployment with the caller's session. The worker re-authenticates that session, re-checks Grant Factory membership for the job's workspace, claims the job with a 5-minute lease and runs the same bounded evidence selection, size guard and model call as before (strategy's model timeout is 180 seconds).
...
- **Usage.** The model call is logged in `gf_ai_runs` as before, and the job row records model, input and output tokens, request size and the selected record ids and reasons, never research text.
