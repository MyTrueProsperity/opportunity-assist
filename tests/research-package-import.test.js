...
function bundle(){
 const v='SYNTHETIC_CB_V1',source_url='https://example.org/research';
 const e={package_version:v,record_id:'CB-001',topic:'Synthetic research',finding:"Apostrophe's literal \\n stays text",approved_language:'Synthetic claim',evidence_level:'B',confidence:'Synthetic',evidence_domain:'RESEARCH',geography:'Study settings',geography_scope:['RESEARCH'],funding_tags:['YOUTH_DEVELOPMENT'],qa_flags:[],source_url,source_org:'Synthetic publisher',year:'2026',population:'Synthetic',methodology:'Synthetic',supports:['Context'],does_not_support:['Local prevalence'],prohibited_language:['Guaranteed outcomes'],verification_status:'PRIMARY_VERIFIED',last_verified:'2026-09-23',review_before_external_use:false};
 return {package_version:v,metadata:{package_version:v},master:'Synthetic master 🍀',tables:{evidence_records:[e],statistics:[{package_version:v,stat_id:'CB-SS-01',record_id:e.record_id,value:null,unit:'qualitative',verification_status:e.verification_status,review_before_external_use:false}],claim_rules:[{package_version:v,rule_id:'CB-CR-01',code:'GEOGRAPHY',severity:'block',enforcement:'review',rule:'Keep study geography'}],funder_packets:[{package_version:v,packet_id:'CB-FP-01',name:'Youth',prohibited_claims:'Do not infer local prevalence',funding_tags:e.funding_tags,priority_evidence_ids:[e.record_id],research_evidence_ids:[e.record_id],strongest_statistic_ids:['CB-SS-01'],claim_rule_ids:['CB-CR-01']}],source_library:[{package_version:v,source_url}],research_sections:[{package_version:v,section_id:'CB-SEC-001',title:'Synthetic',source_locator:'Synthetic / section 1',content_markdown:'Background only',source_urls:[source_url],retrieval_policy:'background_only'}],source_review_queue:[]}};
}
test('package validation rejects broken references and unverified drafting claims',()=>{
 assert.doesNotThrow(()=>validate(bundle()));
 let p=bundle();p.tables.statistics[0].record_id='CB-999';assert.throws(()=>validate(p),/Orphan statistic/);
 ...
 p=bundle();p.tables.evidence_records[0].record_id='CBX-001';assert.throws(()=>validate(p));
});
test('original CTE namespaces survive import without accepting arbitrary identifiers',()=>{
 for(const kind of ['LOCAL','RESEARCH','EMPLOYER','FL','ACCESS','REGIONAL','FUTURE','POLICY']){
  const p=JSON.parse(JSON.stringify(bundle()).replaceAll('CB-001',`CTE_${kind}_001`));
  assert.doesNotThrow(()=>validate(p));
 }
 for(const id of ['CTE_UNKNOWN_001','CTE_LOCAL_1','CTE_LOCAL_001_extra']){
  const p=JSON.parse(JSON.stringify(bundle()).replaceAll('CB-001',id));assert.throws(()=>validate(p));
 }
});
test('generated private package loads resumably, preserves payloads, and requires separate activation',async()=>{
 ...
 try{
  ...
  assert.equal((await get()).records[0].record_id,'CB-001');
