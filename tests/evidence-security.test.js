...
test('displayed deadline hints retain the original quote and cannot invent a year',()=>{const p={...page,text:'Deadline: March 19th'};const c=validateExtraction({deadline_mentioned:claim('2026-03-19','Deadline: March 19th')},p,'FL');assert.equal(c.current_deadline,null);assert.equal(c.deadline_mentioned,'Deadline: March 19th');});
test('exact primary-page names and explicit application notices recover omitted structured facts',()=>{
 const p={...page,text:'About the In Paradise For Good Annual Grants Program. Applications are now open, and close at NOON on September 30, 2099.'};
 const c=validateExtraction({program_name:claim('In Paradise For Good Annual Grants Program','A mistranscribed surrounding excerpt'),application_status:claim('Open','Applications are now open, and close at NOON on September 30, 2099.'),deadline_mentioned:claim('2099-09-30','Applications are now open, and close at NOON on September 30, 2099.')},p,'FL');
 assert.equal(c.program_name,'In Paradise For Good Annual Grants Program');assert.equal(c.evidence.program_name.quote,c.program_name);assert.equal(c.current_cycle_open,true);assert.equal(c.current_status,'ACTIVE_OPEN');assert.equal(c.current_deadline,'2099-09-30');
 const ambiguous=validateExtraction({deadline_mentioned:claim('2099-09-30','Opens September 1, 2099 and closes September 30, 2099')},{...p,text:'Opens September 1, 2099 and closes September 30, 2099'},'FL');assert.equal(ambiguous.current_deadline,null);
});
