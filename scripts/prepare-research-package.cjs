...
const RECORD_ID=/^(?:(?:CFSC|CB|AM|EP|EM|NC|GW|CNE|YW)-[0-9]{3}|CTE_(?:LOCAL|RESEARCH|EMPLOYER|FL|ACCESS|REGIONAL|FUTURE|POLICY)_[0-9]{3}|BM-ENT-V1-(?:E[0-9]{4}|S[0-9]{3}))$/;
...
function validate(p){
 ...
 for(const r of p.tables.evidence_records){
  assert(RECORD_ID.test(r.record_id),'Unsupported record ID: '+r.record_id);assert(['A','B','C'].includes(r.evidence_level));
...
module.exports={readBundle,validate,prepare,RECORD_ID};
