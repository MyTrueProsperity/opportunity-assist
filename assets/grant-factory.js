  ...
  function mount(main, sb, opportunities = []) {
    ...
    function methodologyCards() {
      ...
      return (m ? '<details class="gf-card"><summary>Grant methodology · ' + m.rules.length + ' rules applied to every strategy, draft and audit</summary><p class="gf-note">These rules shape reasoning and wording. They are not evidence and cannot make a claim supportable.</p>' + m.rules.map(r => '<p><strong>' + esc(r.id + ' · ' + r.title) + '</strong><br>' + esc(r.rule) + '</p>').join('') + '<p class="gf-meta">' + esc(m.version) + '</p></details>' : '') +
        ...
        (fw?.evidence_crosswalk?.entries?.length ? '<details class="gf-card"><summary>Evidence crosswalk · ' + fw.evidence_crosswalk.entries.length + ' program components</summary><p class="gf-note">Points each program component to research records in this library. A listed record is a place to look, not proof: it supports a draft only when cited and marked verified. External research never establishes a program\'s own outcomes.</p>' + fw.evidence_crosswalk.entries.map(e => '<div class="gf-list-row"><strong>' + esc(e.component) + '</strong>' + (e.status ? ' ' + pill(e.status) : '') + '<p>' + esc(e.proposition || '') + '</p>' + (e.families?.length ? '<p><strong>Evidence families:</strong> ' + esc(e.families.join(', ')) + '</p>' : '') + (e.competencies?.length ? '<p><strong>Competencies:</strong> ' + esc(e.competencies.join(', ')) + '</p>' : '') + '<p><strong>Records:</strong> ' + esc((e.records || []).join(', ')) + '</p>' + (e.do_not_claim ? '<p><strong>Do not claim:</strong> ' + esc(e.do_not_claim) + '</p>' : '') + '</div>').join('') + '<p class="gf-meta">' + esc(fw.evidence_crosswalk.version || '') + '</p></details>' : '') +
        (fw?.narrative_guidance?.length ? '<details class="gf-card"><summary>Narrative guidance · ' + fw.narrative_guidance.length + '</summary><p class="gf-note">How to frame a need case. This is interpretation, not a statistic; cite the listed records for any number.</p>' + fw.narrative_guidance.map(g => '<div class="gf-list-row"><strong>' + esc(g.title || g.id) + '</strong><p>' + esc(g.guidance || '') + '</p>' + bullets(g.steps) + (g.records?.length ? '<p><strong>Records:</strong> ' + esc(g.records.join(', ')) + '</p>' : '') + (g.do_not_claim ? '<p><strong>Do not claim:</strong> ' + esc(g.do_not_claim) + '</p>' : '') + '</div>').join('') + '</details>' : '') +
    ...
    // Research-gap registers, population bundles and withheld claims recorded in
    // a package's metadata. Review material only; none of it is evidence.
    function packageGuidance(packages) {
      const cards = [];
      for (const p of packages || []) {
        const m = p.metadata || {};
        const name = m.title || p.package_version;
        if (m.evidence_gaps?.length) cards.push('<details class="gf-card"><summary>Research gaps · ' + m.evidence_gaps.length + ' open questions · ' + esc(name) + '</summary><p class="gf-note">Evidence still needed. Name these as gaps in a proposal rather than filling them.</p>' + m.evidence_gaps.map(g => '<div class="gf-list-row"><strong>' + esc((g.gap_id ? g.gap_id + ' · ' : '') + (g.question || '')) + '</strong>' + (g.priority ? ' ' + pill(g.priority) : '') + '<p>' + esc(g.why_it_matters || '') + '</p><p class="gf-meta">' + esc([g.geography, g.population, g.preferred_source && 'Preferred source: ' + g.preferred_source].filter(Boolean).join(' · ')) + '</p></div>').join('') + '</details>');
        if (m.population_bundles?.length) cards.push('<details class="gf-card"><summary>Population bundles · ' + m.population_bundles.length + ' · ' + esc(name) + '</summary><p class="gf-note">Records to consult for each population. Search a record id above to open it.</p>' + m.population_bundles.map(b => '<div class="gf-list-row"><strong>' + esc(b.population) + '</strong><p>' + esc((b.record_ids || []).join(', ')) + '</p>' + (b.note ? '<p class="gf-note">' + esc(b.note) + '</p>' : '') + '</div>').join('') + '</details>');
        if (m.withheld_claims?.length || m.conflicts_resolved?.length) cards.push('<details class="gf-card"><summary>Use carefully · withheld claims and resolved conflicts · ' + esc(name) + '</summary>' + (m.conflicts_resolved || []).map(c => '<div class="gf-list-row"><strong>' + esc(c.topic) + '</strong><p>' + esc(c.resolution) + '</p></div>').join('') + (m.withheld_claims || []).map(c => '<div class="gf-list-row"><strong>Withheld: ' + esc(c.claim) + '</strong><p>' + esc(c.reason || '') + '</p>' + (c.see ? '<p class="gf-meta">See ' + esc(c.see) + '</p>' : '') + '</div>').join('') + '</details>');
      }
      return cards.join('');
    }
    function renderResearch(el) {
      ...
      el.innerHTML = '<div class="gf-row"><div><h2>Research Library</h2><p class="gf-note">Community context and intervention research. Only verified records are eligible for drafting. Every claim still requires review.</p></div></div>' +
        ...
        methodologyCards() + packageGuidance(library.packages) +
