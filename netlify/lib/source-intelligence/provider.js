...
function responseFailure(message,data){const error=new Error(message);error.usage=data.usage;error.cost=data.cost;return error;}
function createProvider(env=process.env,fetcher=fetch) {
  ...
  return {model:MODEL,
    async search(queries,stateName){
      ...
      if(errors.length&&!leads.length)throw responseFailure('Web search tool failed: '+(errors[0].content?.error_code||'unavailable'),data);
      if(!resultBlocks.length)throw responseFailure('Provider did not execute a verifiable web search',data);
    ...
    async extract(page,state){
      const data=await message({max_tokens:9000,system:SYSTEM,messages:[{role:'user',content:JSON.stringify({today:new Date().toISOString().slice(0,10),target_state:state,url:page.url,links:page.links.slice(0,80),page_text:page.text})}]});
      if(data.stop_reason==='max_tokens')throw responseFailure('Extraction exceeded response limit; investigate page',data);
      const text=(data.content||[]).filter(b=>b.type==='text').map(b=>b.text).join('');let raw;
      try{raw=JSON.parse(text.replace(/^```(?:json)?\s*|\s*```$/g,''));}catch{throw responseFailure('Invalid extraction JSON; no claims accepted',data);}
      if(!Array.isArray(raw.programs))throw responseFailure('Invalid extraction schema',data);
